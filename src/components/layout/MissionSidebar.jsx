...
const WaypointCard = ({ wp, index, onUpdate, onRemove }) => (
  <motion.div
    layout
    ...
    >
      {AXES.map((axis, axisIdx) => (
        ...
        >
          ...
          <input
            type="number"
            value={wp.coords[axisIdx]}
            onChange={(e) => onUpdate(wp.id, axisIdx, Number(e.target.value))}
            style={{ width: "100%" }}
          />
        </div>
      ))}
...
export function MissionSidebar({
  ...
  const handleAdd = () => {
    ...
    const lastCoords = last?.coords ?? [0, 0, 0];
    addWaypoint({
      id: Date.now(),
      label: `WP${waypoints.length}`,
      coords: [lastCoords[0] + 15, lastCoords[1], lastCoords[2]],
    });
  };
