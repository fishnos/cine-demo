...
export const RAW_PATH = WAYPOINTS.map(w => ({ x: w.coords[0], y: w.coords[1] }))
...
export function useTelemetry() {
  ...
  useEffect(() => {
    const wpIdx = Math.min(Math.floor(t * WAYPOINTS.length), WAYPOINTS.length - 1)
    if (wpIdx > lastWP.current) {
      lastWP.current = wpIdx
      const wp   = WAYPOINTS[wpIdx]
      const mins = String(Math.floor(t * 10)).padStart(2, '0')
      setEvents(prev => [
        { id: eventId.current++, time: `00:${mins}`, text: `Reached ${wp.label}`, type: 'success' },
        ...prev.slice(0, 19),
      ])
    }
  }, [t])
